...
bootfs_mount() {
  exec &>/dev/null
  printf "extrott-attach: running bootfs attach pre-init script..." > /dev/kmsg
  ...
  if [ $? -eq 0 ]; then
    printf "extrott-attach: successfully mounted the boot file system" > /dev/kmsg
    eval "$(cat $OVERLAY_UEVENT)"
    OVERLAY_DEVICE=/dev/$DEVNAME
    mknod_dev $OVERLAY_DEVICE $OVERLAY_NODE_TYPE
    if [ ! -f "$BOOTFS_TRIG" ]; then
      printf "extrott-attach: moving backup to overlay file system" > /dev/kmsg
      ...
      if [ $? -eq 0 ]; then
        printf "extrott-attach: successfully mounted the new overlay" > /dev/kmsg
        rm -rf $OVERLAYFS
        if [ -f "$BOOTFS_TARZ" ]; then
          printf "extrott-attach: found the sysupgrade backup file!" > /dev/kmsg
          cp -ar $BOOTFS_TARZ $OVERLAYFS
        else
          printf "extrott-attach: sysupgrade backup file not found!" > /dev/kmsg
        fi
        rm -rf $BOOTFS
        printf "extrott-attach: configuring the default extroot" > /dev/kmsg
  ...
  if [ $? -eq 0 ]; then
    printf "extrott-attach: successfully mounted pstore" > /dev/kmsg
  fi
}
