...
function compressors (o) {
	...
	o.value('lz4',     '2-4: lz4');
	o.value('lzo',     '3-3: lzo');
	o.value('lz4hc',   '4-6: lz4hc');
	o.value('zstd',    '5-1: zstd');
	o.value('deflate', '6-2: deflate');
}
	...
	render: function(loaded) {
		...
		if (zr) {
			...
			o = s.option(form.ListValue, 'algorithm', _('Compression algorithm'),
				_('Select the compression algorithm.<ul><li>The first number represents the relative speed; the lower the number, the faster<li>The second number means the relative compression performance; the lower the number, the more savings</ul>'));
			compressors(o);
			o.default	= 'lzo_rle';
			o.depends('enabled', '1');
			o.rmempty	= false;
			...
			o = s.option(form.Value, 'pool_limit', _('Absolute memory pool limit'),
				_('For calculating the disk\'s size (which is different from the estimated size occupied by it when filled with data in memory), never size the disk bigger than this hard limit.<br>This limit is a percentage of the total system memory.'));
		...
		if (zs) {
			...
			o = s.option(form.Flag, 'enabled', _('Enable a zswap cache for swap devices'),
				_('Enable the compressed swap cache pool, a swap cache for all of the system\'s swap areas.'));
			o.default	= true;
			o.rmempty	= false;
